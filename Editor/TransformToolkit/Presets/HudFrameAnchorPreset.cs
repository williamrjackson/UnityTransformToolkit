...
{
    [CreateAssetMenu(
        menuName = "Transform Toolkit/Presets/HUD Frame Anchor (RectTransform → 3D)",
    ...
    public sealed class HudFrameAnchorPreset : TransformPreset
    {
        ...
        public override void Apply(PresetContext ctx, Transform[] targets)
        {
            ...
            for (int i = 0; i < targets.Length; i++)
            {
                ...
                if (faceCamera)
                {
                    Vector3 forward = cam.transform.position - tr.position;
        ...
        private static bool TryGetRectScreenCorners(RectTransform rect, Camera cam, out Vector2 screenMin, out Vector2 screenMax)
        {
            ...
            // Overlay canvas corners are already expressed in screen space and must not be
            // projected through the 3D camera. Camera-space and world-space canvases do need
            // their rendering camera for the conversion.
            Canvas canvas = rect.GetComponentInParent<Canvas>();
            Camera projectionCamera = canvas && canvas.rootCanvas.renderMode == RenderMode.ScreenSpaceOverlay
                ? null
                : cam;
            ...
            // Use every corner so rotated and flipped RectTransforms produce a valid
            // screen-space axis-aligned bounding box.
            for (int i = 0; i < wc.Length; i++)
            {
                Vector2 corner = RectTransformUtility.WorldToScreenPoint(projectionCamera, wc[i]);
        ...
        private static float ComputeDepth(Camera cam, Transform tr, DepthMode mode, float fixedDist)
        {
            ...
            switch (mode)
            {
                case DepthMode.FixedDistanceFromCamera:
                    return Mathf.Max(cam.nearClipPlane + 0.01f, fixedDist);
                ...
                default:
                    ...
                    // If the object is behind/on the camera, place it safely beyond the
                    // near plane to avoid projection flips.
                    return Mathf.Max(cam.nearClipPlane + 0.01f, z);
            }
        }
